...
const connectWithRetry = async () => {
  ...
  while (retries) {
    try {
      ...
      console.error(`❌ Erreur de connexion MongoDB (${retries} tentatives restantes):`, err.message);
      retries -= 1;
      if (retries === 0) {
        console.error('💡 Impossible de se connecter à MongoDB. Vérifiez que:');
        console.error('   1. MongoDB est installé et démarré (mongod)');
        console.error('   2. L\'URI de connexion est correcte');
        console.error('   3. Votre adresse IP est autorisée (pour Atlas)');
        process.exit(1);
      }
